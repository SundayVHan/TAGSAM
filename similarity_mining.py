...
class MultilabelContrastiveLoss(nn.Module):
    ...
    def forward(self, logits, gt_matrix):
        ...
        if self.loss_type == "KL":
            tau = 1/0.07
            student_probs = torch.sigmoid(logits / tau)
            teacher_probs = torch.sigmoid(gt_matrix / tau)

            # Construct probability vectors [p, 1-p]
            student_probs_vector = torch.stack([student_probs, 1 - student_probs], dim=-1)
            teacher_probs_vector = torch.stack([teacher_probs, 1 - teacher_probs], dim=-1)

            # Compute KL divergence for each class and each sample
            kl_divergence = F.kl_div(
                student_probs_vector.log(),
                teacher_probs_vector,
                reduction='batchmean'
            )

            # Scale by tau^2
            binary_kl_loss = kl_divergence

            return binary_kl_loss
...
def calculate_cmi_emp(S, labels, num_classes):
    N = S.size(0)
    cmi_emp = 0.0
    labels = torch.tensor(labels, device=S.device)

    for y in range(num_classes):
        indices_true = (labels == y).nonzero(as_tuple=True)[0]
        # indices_false = (labels != y).nonzero(as_tuple=True)[0]
        # S_y = S[indices_true][:, indices_false]
        S_y = S[indices_true]
        if S_y.size(0) == 0:
            continue
        S_y = F.softmax(S_y, dim=1)

        Q_y_emp = S_y.mean(dim=0)

        kl_divs = F.kl_div(S_y.log(), Q_y_emp, reduction='batchmean')

        cmi_emp += kl_divs * S_y.size(0)
    ...
    #     indices = (labels == y).nonzero(as_tuple=True)[0]
    #     S_y = S[indices]
    #
    #     if S_y.size(0) == 0:
    #         continue
    ...
    #     Q_y_emp = S_y.mean(dim=0)
    #
    #     # Calculate KL divergence for each sample in S_y
    #     kl_divs = F.kl_div(S_y.log(), Q_y_emp, reduction='batchmean')
    #
    #     # Accumulate the CMI_emp
    #     cmi_emp += kl_divs * S_y.size(0)
