...
class ReparamModule(nn.Module):
    ...
    def get_params_mask_for_module(self, module_name, flat_param):
        mask = torch.zeros_like(flat_param, dtype=torch.bool)
        current_index = 0

        for (mn, n), numel in zip(self._param_infos, self._param_numels):
            if module_name in mn:
                mask[current_index:current_index + numel] = True
